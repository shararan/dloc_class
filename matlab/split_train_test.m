function [train_idxs, test_idxs] = split_train_test(labels, bbox)
    %{
    ...
    Args:
        ...
        bbox (vector): [x_min, x_max, y_min, y_max] bounding box for selecting test data.
    ...
    % end
    test_mask = (labels(:,1) >=  bbox(1)) & (labels(:,1) <=  bbox(2)) & ...
                (labels(:,2) >=  bbox(3)) & (labels(:,2) <=  bbox(4));
