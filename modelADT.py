...
#        self.gpu_ids = opt.gpu_ids
    ...
    def set_data(self, input, map_input, target, map_target, convert=False, shuffle_channel=False):
        shape_in = input.shape
        if shuffle_channel:
            self.input = input[:,torch.randperm(shape_in[1]),:,:]
            self.map_input = map_input[:,torch.randperm(shape_in[1]),:,:]
        else:
            self.input = input
        ...
        if convert:
            self.input, self.target, self.map_input, self.map_target = Variable(self.input), Variable(self.target), Variable(self.map_input), Variable(self.map_target)

        self.input = self.input.to(self.device)
        self.map_input = self.map_input.to(self.device)        
        self.target = self.target.to(self.device)
        self.map_target = self.map_target.to(self.device)

    def forward(self):
        self.output = self.net(self.input,self.map_input)
        if self.opt.loss_type != "NoLoss":
            self.loss = self.loss_weight*self.loss_criterion(self.output, self.target)
